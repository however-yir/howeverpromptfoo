...
// Fourthwall storefront token (public storefront credential).
// Configure it through environment variable to avoid committing credentials.
...
async function apiFetch<T>(endpoint: string, options?: RequestInit): Promise<T> {
  if (!STOREFRONT_TOKEN) {
    throw new Error(
      'Missing REACT_APP_FOURTHWALL_STOREFRONT_TOKEN. Please configure the storefront token in your environment.',
    );
  }
