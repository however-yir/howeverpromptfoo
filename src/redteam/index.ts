...
import { findRegisteredPlugin, getRegisteredPlugins } from './registry';
...
export async function synthesize({
  ...
  const registeredPluginKeys = new Set(
    getRegisteredPlugins().map((pluginFactory) => pluginFactory.key),
  );
  const expandPlugin = (
  ...
  plugins.forEach((plugin) => {
    ...
    const isDirectPlugin = registeredPluginKeys.has(plugin.id);
  ...
  const validatePlugin: (plugin: (typeof plugins)[0]) => boolean = (plugin) => {
    ...
    const registeredPlugin = findRegisteredPlugin(plugin.id);
  ...
  await async.forEachLimit(plugins, maxConcurrency, async (plugin) => {
    ...
    const { action } = findRegisteredPlugin(plugin.id) || {};
