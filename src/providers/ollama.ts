...
import { buildBearerAuthHeaders, resolveProviderBaseUrl } from './shared/providerTransport';
...
export class OllamaCompletionProvider implements ApiProvider {
  ...
  private async callApiInternal(prompt: string): Promise<ProviderResponse> {
    ...
    try {
      const apiBaseUrl = resolveProviderBaseUrl(
        ['HOWEVER_OLLAMA_BASE_URL', 'OLLAMA_BASE_URL'],
      ...
      response = await fetchWithCache<string>(
        ...
        {
          method: 'POST',
          headers: buildBearerAuthHeaders('OLLAMA_API_KEY', {
