...
describe('OpenAI Provider', () => {
  describe('OpenAiGenericProvider', () => {
    ...
    it('should prioritize HOWEVER_OPENAI_BASE_URL over OPENAI_BASE_URL', () => {
      process.env.HOWEVER_OPENAI_BASE_URL = 'https://however.gateway.local/v1';
      process.env.OPENAI_BASE_URL = 'https://fallback.example.com/v1';

      const envProvider = new OpenAiGenericProvider('test-model');
      expect(envProvider.getApiUrl()).toBe('https://however.gateway.local/v1');
    });

    it('should keep OPENAI_BASE_URL url format valid', () => {
      process.env.OPENAI_BASE_URL = 'https://api.example.com/v1';
      const envProvider = new OpenAiGenericProvider('test-model');
      expect(() => new URL(envProvider.getApiUrl())).not.toThrow();
    });
