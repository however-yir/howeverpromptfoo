...
describe('OllamaCompletionProvider', () => {
  ...
  it('should prioritize HOWEVER_OLLAMA_BASE_URL when present', async () => {
    process.env.HOWEVER_OLLAMA_BASE_URL = 'http://127.0.0.1:21434';
    const mockResponse = {
      data: '{"response":"test response","done":true}\n',
      cached: false,
      status: 200,
    ...
    expect(vi.mocked(fetchWithCache).mock.calls[0]?.[0]).toContain('http://127.0.0.1:21434');

    delete process.env.HOWEVER_OLLAMA_BASE_URL;
  });
});
