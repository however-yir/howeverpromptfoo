...
const createDefaultCitation = (packageJson: PackageJson): Citation => ({
  ...
  title: 'howeverpromptfoo',
  version: packageJson.version,
  'date-released': new Date().toISOString().slice(0, 10),
  url: 'https://github.com/however-yir/howeverpromptfoo',
  'repository-code': 'https://github.com/however-yir/howeverpromptfoo',
  license: packageJson.license,
  type: 'software',
  description: packageJson.description,
  keywords: ['howeverpromptfoo', 'llm', 'evaluation', 'testing', 'prompt-engineering', 'red-team'],
});
...
async function getReleaseDate(version: string): Promise<string | null> {
  try {
    ...
    const response = await fetch(
      `https://api.github.com/repos/however-yir/howeverpromptfoo/releases/tags/${version}`,
    );
    if (!response.ok) {
